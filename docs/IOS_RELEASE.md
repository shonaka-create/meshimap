...
| API | 推奨 日次上限 | 月換算 | 根拠 |
|---|---|---|---|
| Geocoding API | **300** | 9,000 | 無料枠 10,000/月 に収まる |
| Maps SDK for iOS | **設定しない** | — | 上限に達すると地図が出なくなり、通常利用者の体験を壊すため |

Geocoding は**投稿1件につき1回**だけ消費します（地図の閲覧では消費しません）。
1日300投稿はリリース初期には十分で、足りなくなってから上げれば済みます。
...
|---|---|---|---|---|
| Maps SDK for iOS (Dynamic Maps) | Essentials | 10,000 | 約 $7 / 1,000 | 地図画面を開くたび |
| Geocoding | Essentials | 10,000 | 約 $5 / 1,000 | 投稿1件につき1回 |

**月10,000投稿・1日約330起動までは完全に無料**です。

> 参考: 最寄り駅で絞る機能を将来入れたくなった場合、Places API
> （Pro / 無料枠5,000 / 約$32per1,000）を使うより、
> 駅座標のオープンデータ（OpenStreetMap や 国土数値情報）を
> Supabase に取り込んで SQL で最近傍を求める方が、
> 費用ゼロ・高速・高精度です。
...
- [ ] 県 → 市区町村 → 駅 とタップで下れて、下ると上の階層のバブルが消える
- [ ] 投稿で写真が5枚まで選べ、**動画が選択肢に出ない**
- [ ] 投稿後、都道府県・市区町村・最寄り駅が自動で入っている（Supabase の posts を確認）
- [ ] 投稿が初期状態で非公開になっている
- [ ] 通報・ブロック・アカウント削除が動く
...
| 都道府県が入らない | Geocoding API が未有効。`resolveRegion` は失敗しても投稿は通る設計なので気づきにくい |
| 最寄り駅が常に空 | Places API が未有効。駅は「理想」要件なので null でも動作する |
