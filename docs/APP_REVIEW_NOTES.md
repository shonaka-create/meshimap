...
実際にログインできることを確認済みのアカウント（2026-09-04 に再検証）:

| メールアドレス | パスワード | 中身 |
|---|---|---|
| `teardoro+taro@gmail.com` | `.env.local` の `SEED_PASSWORD` | @ebichan / えびちゃん / 投稿7件 |

> ⚠ **メールアドレスは `+taro` のまま変わっていません。** アカウント自体は同じで、
> 表示名とユーザーIDだけを `@taro`（田中太郎）から `@ebichan`（えびちゃん）に
> 変えてあります。ログインに使うのは引き続きこのアドレスです。
>
> このアカウントはデモではなく、実際に運用している本アカウントです。
> 中身は神楽坂周辺のイタリアン／フレンチ7件。詳しくは
> `SETUP.md` の「4-0.5. 本アカウント @ebichan」を参照。
>
> 他のデモ用ユーザー（hanako / kenji / yuki / yamada / ebisu / admin）は、
> 同じパスワードではログインできませんでした。**審査には上のアカウントを使ってください。**
