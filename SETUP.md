...
| 15 | `supabase/migrations/0014_review_fixes.sql` | Storage を自分のフォルダに限る・ランキングのブロック迂回・承認待ちが地図の枠を食う問題 |
| 16 | `supabase/migrations/0015_follow_lists.sql` | フォロー関係の閲覧を絞り、フォロー/フォロワー一覧を関数経由にする |
| 17 | `supabase/migrations/0016_post_image_limit.sql` | 1投稿の写真を5枚までにDB側でも止める |
| 18 | `supabase/migrations/0017_region_counts_by_genre.sql` | 地域ごとの件数をジャンルで絞れるようにする |
...
>
> **`0014` は公開前に必ず流してください（セキュリティ）。**
> これを流すまで、ログインさえしていれば誰でも Storage の任意のパスへ
> ファイルを置けます（両バケットは public）。あわせて、ブロックした相手が
> 月間ランキングに出てしまう問題と、承認待ちのフォローが
> 「地図に出せる人数」の枠を食う問題も直ります。
>
> **`0012` を既に流してあるDBは、もう一度流し直してください。**
> 禁止語の一覧に抜けがあったため語を足しています（冪等なので何度でも可）。
>
> **`0015` もアプリを更新する前に流してください。**
> プロフィールの「フォロワー」「フォロー中」の数字から一覧を開けるようにしたので、
> アプリは `followers_of` / `following_of` を呼びます。流していないDBでは
> 一覧が常に空になります。あわせて `follows` の閲覧を「自分が関わる行」に絞ります
> （それまでは誰でも全部読めました）。
>
> **`0017` はアプリを更新する前に流してください。**
> 地図のジャンル絞り込みを全階層で使えるようにしたので、アプリは
> `post_counts_by_region` を3引数で呼びます。引数を足しただけなので
> 古いアプリ（2引数）はそのまま動きますが、流していないDBに
> 新しいアプリを繋ぐと地域の集計が取れず、バブルが出ません。
>
> `0016` はいつ流しても構いません（アプリの挙動は変わりません）。
>
> ⚠ **18 まで必ず実行してください。**
...
> （`@hanako` 佐藤花子など）の名義で載ります。
>
> ℹ️ `@taro`（田中太郎）は、この一覧から外れています。
> あのアカウントは `@ebichan`（えびちゃん）として実際に運用する
> 本アカウントに切り替えました。担当は `scripts/seed-ebichan.mjs` で、
> `npm run seed` は一切触りません。詳しくは下の
> 「本アカウント @ebichan」を参照。
>
...
### 4-0.5. 本アカウント @ebichan

`@ebichan`（えびちゃん）は**デモではなく、実際に運用するアカウント**です。
もともと seed が作った `@taro`（田中太郎）を作り替えたもので、
中身は神楽坂周辺のイタリアン／フレンチ7件です。

担当スクリプトは `scripts/seed-ebichan.mjs` の1本だけ。
`npm run seed` はこのアカウントに一切触りません
（`scripts/seed.mjs` の `ALL_USERS` から外してあります）。
...
書き込む前に、投稿1件ごとのジャンル・価格帯・シチュエーションが
`theme.ts` の選択肢にあるか、座標から判定したエリアが記載と一致するか、
写真URLが生きているかを検査します。1件でも合わなければ何も書き込みません。

店名が一致するかどうかで見ているので、何度流しても結果は同じです（冪等）。
`POSTS` から消した店は、次に流したときにDBからも消えます。

> ⚠ **「デモ用のアカウントです」の帯を消すには SQL Editor が要ります。**
> `is_demo` はアプリ側からは外せません（移行0010 の `protect_demo_flag`）。
> `supabase/migrations/0018_ebichan_real_account.sql` を SQL Editor で実行してください。
> 移行 `0010` の対象リストからも `taro` を外してあるので、
> `0010` を流し直してもデモ印は戻りません。

> ⚠ **`0010` の対象リストに `taro` / `ebichan` を書き戻さないこと。**
> 書き戻すと、`0010` を流した瞬間に本アカウントへデモの帯が復活します。
