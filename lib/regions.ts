 ...
 * 日本の都道府県と主要エリアの座標データ。
 *
 * ★ このファイルは mobile/src/lib/regions.ts と同一内容を保つこと。
 *   Web(Next.js) と Mobile(Expo) は別の npm プロジェクトで、
 *   web の tsconfig は mobile/ を exclude しているため直接 import できない。
 *   ずれると Web と Mobile で同じ座標が別のエリア名になるので、
 *   `npm run check:regions` が CI で差分を検出する。
 *
