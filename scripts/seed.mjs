...
const POSTS_BY_USER = {
  taro: [
    /* ────────────────────────────────────────────────
     * ★ ここは実在の店。評価は一律★5で登録している。
     *
     *   デモである以上、本人が行っていない店の評価が
     *   実在の店に紐づく。公開前に、運営名義にするか
     *   デモである旨を明示するかを決めること。
     *
     *   座標は「その店がある街区」の目安であって、
     *   建物単位の精度は無い。エリア判定（最寄り8km）には
     *   十分だが、地図で番地まで合わせたいときは要修正。
     *   npm run check:seed で、座標とエリアの食い違いは検出できる。
     * ──────────────────────────────────────────────── */

    /* ── 東京 / ラーメン ───────────────────────── */
    {
      caption: '巣鴨の醤油そば。黒トリュフの香りが立ちのぼって、これがラーメンかと思わされる一杯。ミシュランに載ってからも並びが落ち着かないが、並ぶ価値がある',
      rating: 5,
      genre: 'ラーメン',
    ...
    {
      caption: '中野の老舗。魚介と動物系を合わせるダブルスープの元祖と言われる店で、今食べても古びていない。この一杯から派生した店がどれだけあるかを考えると、背筋が伸びる',
      rating: 5,
      genre: 'ラーメン',
      price_range: '〜¥1,000',
    ...
    {
      caption: '三田の本店。すべてがここから始まった場所で、行列も作法もひっくるめて一度は経験しておくべきだと思う。小でも十分すぎる量。コールは落ち着いて',
      rating: 5,
      genre: 'ラーメン',
      price_range: '〜¥1,000',
    ...
    {
      caption: '新宿御苑の蛤の貝出汁。ここに白トリュフが合わさって、澄んでいるのに濃い。カウンターだけの小さな店で、一杯に対する集中の仕方が違う',
      rating: 5,
      genre: 'ラーメン',
      price_range: '¥1,001〜¥3,000',
    ...
    {
      caption: '和出汁の効いた一杯と、日替わりの限定が楽しみな店。ラーメンというより日本料理に近い設計で、器の中がとにかく静か。開店前から人が並んでいる',
      rating: 5,
      genre: 'ラーメン',
      price_range: '¥1,001〜¥3,000',
      location_name: '饗 くろ喜',
     ...
     */
    {
      caption: '神田須田町の老舗蕎麦。建物からして時間が積もっている。もりを手繰って、板わさで一本つける昼が最高に贅沢。せいろの角が立っていて、汁は濃いめの江戸前',
      rating: 5,
      genre: '和食',
      price_range: '¥1,001〜¥3,000',
      location_name: 'まつや',
    ...
    {
      caption: '清澄白河の自家焙煎。豆ごとの説明が的確で押しつけがましくない。この街がコーヒーの街になる前からやっている店で、界隈の空気の作り手だと思う',
      rating: 5,
      genre: 'カフェ',
      price_range: '〜¥1,000',
    ...
    {
      caption: '予約が取れないことで知られる鮨。実際に座ると、握りの速さと間の取り方に驚く。シャリの温度がずっと一定で、最後まで緊張感が切れない。記念日にどうぞ',
      rating: 5,
      genre: '寿司',
      price_range: '¥10,001〜',
    ...
    {
      caption: '神保町の讃岐うどん。ひやかけの出汁が澄んでいて、コシというより弾力。昼は必ず並ぶが回転が速い。天ぷらは揚げ置きしないので、待つ甲斐がある',
      rating: 5,
      genre: '和食',
      price_range: '〜¥1,000',
      location_name: '丸香',
    ...
    {
      caption: '銀座のイタリアン。ウニのスパゲッティが有名だが、前菜からデザートまで隙がない。値段の割に満足度が高く、記念日にも普段にも使える稀な店。予約は早めに',
      rating: 5,
      genre: 'イタリアン',
    ...
    /* ── 大阪 ─────────────────────────────────
     * 都道府県をまたがないと、エリア集計も
     * 県からエリアへ降りる演出も確かめられない。
     */
    {
      caption: 'きつねうどん発祥と言われる店。甘く炊いたお揚げと、関西らしい澄んだ出汁。名物のおじやうどんは、うどんの後に雑炊が来るような一杯で、大阪でしか成立しない発明だと思う',
      rating: 5,
      genre: '和食',
      price_range: '〜¥1,000',
    ...
    {
      caption: '鶴橋の老舗焼肉。駅を降りた瞬間から街全体が煙の匂いで、着く前から食べる気になっている。ホルモンの鮮度が段違いで、タレより塩で食べたくなる',
      rating: 5,
      genre: '焼肉',
    ...
    {
      caption: '北新地の日本料理。カウンター越しに一品ずつ出てくる構成で、椀物がとにかく良かった。足すのではなく引く料理。接待で使ったが、次は一人で来たい',
      rating: 5,
      genre: '和食',
      price_range: '¥10,001〜',
    ...
    {
      caption: '大阪駅前ビルの地下、時間が止まっている喫茶店。金の壁と赤いソファ、この空間ごと文化財だと思っている。新幹線の前に必ず寄ってしまう',
      rating: 5,
      genre: 'カフェ',
      price_range: '〜¥1,000',
    ...
    {
      caption: '新世界の洋食。ヘレカツサンドが有名で、断面の赤さと甘辛いソースが忘れられない。持ち帰りもできるが、店で出来立てを食べたほうがいい。カウンターの佇まいも含めて',
      rating: 5,
      genre: '洋食',
      price_range: '¥3,001〜¥5,000',
      location_name: 'グリル梵',
